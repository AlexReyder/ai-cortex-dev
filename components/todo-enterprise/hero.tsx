"use client"

import Image from "next/image"
import { Shield, Globe, Server, RefreshCw } from "lucide-react"
import { CTAButton } from "@/components/cta-modals"

const trustBullets = [
  { icon: Shield, text: "Российская инфраструктура" },
  { icon: Globe, text: "Web + Mobile + Server" },
  { icon: Server, text: "Private Cloud / On-Prem" },
  { icon: RefreshCw, text: "Migration-friendly логика" },
]

export function TodoEnterpriseHero() {
  return (
    <section className="relative pt-28 pb-20 md:pt-36 md:pb-28 overflow-hidden">
      {/* Background gradient */}
      <div className="absolute inset-0 bg-gradient-to-b from-background via-background to-background-secondary" />
      
      <div className="relative z-10 max-w-6xl mx-auto px-6">
        <div className="grid lg:grid-cols-12 gap-12 lg:gap-8 items-center">
          {/* Left column - Text content */}
          <div className="lg:col-span-5 text-center lg:text-left">
            {/* Eyebrow */}
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-primary/10 border border-primary/20 mb-6">
              <span className="text-xs font-medium text-primary tracking-wide">ToDo Enterprise</span>
            </div>

            {/* Headline */}
            <h1 className="text-3xl md:text-4xl lg:text-[2.75rem] font-semibold text-foreground leading-[1.15] tracking-tight mb-5 text-balance">
              Российская система управления задачами и проектами для enterprise
            </h1>

            {/* Subheadline */}
            <p className="text-base md:text-lg text-foreground-secondary leading-relaxed mb-4 text-pretty max-w-xl mx-auto lg:mx-0">
              Привычный сценарий работы для команд, которым нужен надежный переход с западных систем без потери управляемости.
            </p>

            {/* Supporting text */}
            <p className="text-sm text-foreground-muted leading-relaxed mb-8 max-w-lg mx-auto lg:mx-0">
              Kanban, Scrum, roadmap, контроль сроков, ролей и процессов — в единой системе с безопасным размещением и возможностью внедрения в контуре клиента.
            </p>

            {/* CTAs */}
            <div className="flex flex-col sm:flex-row gap-3 justify-center lg:justify-start mb-6">
              <CTAButton
                variant="primary"
                size="lg"
                modalType="todo-demo"
              >
                Попробовать ToDo
              </CTAButton>
              <CTAButton
                variant="secondary"
                size="lg"
                modalType="todo-stand"
              >
                Запросить демо-стенд
              </CTAButton>
            </div>

            {/* Secondary CTA */}
            <div className="flex justify-center lg:justify-start">
              <CTAButton
                variant="ghost"
                size="sm"
                modalType="implementation"
              >
                Обсудить внедрение →
              </CTAButton>
            </div>

            {/* Trust bullets */}
            <div className="mt-10 pt-8 border-t border-border">
              <div className="grid grid-cols-2 gap-4">
                {trustBullets.map((bullet, index) => (
                  <div key={index} className="flex items-center gap-2.5">
                    <bullet.icon className="w-4 h-4 text-primary shrink-0" />
                    <span className="text-xs text-foreground-muted">{bullet.text}</span>
                  </div>
                ))}
              </div>
            </div>
          </div>

          {/* Right column - Product visual */}
          <div className="lg:col-span-7">
            <div className="relative">
              {/* Ambient glow */}
              <div className="absolute -inset-8 bg-primary/5 rounded-3xl blur-3xl" />
              
              {/* Product image container */}
              <div className="relative rounded-2xl overflow-hidden shadow-2xl shadow-black/20">
                <Image
                  src="/images/demo.png"
                  alt="ToDo Enterprise - современный интерфейс управления задачами с Kanban-доской, timeline и мобильным приложением"
                  width={900}
                  height={600}
                  className="w-full h-auto"
                  priority
                />
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
  )
}
